...
multi_female1and3 <- c(1.46, 0.87, 1.05, 0.64)  # order: union-white, union-black, confed-white, confed-black
...
print("Regional Balance Check:")
print(balance_check)

# checkequal allocations across geos
assert_balanced <- all(balance_check$n == 50)
cat(sprintf("\nBalanced design confirmed: %s\n", assert_balanced))
```
...
```{summary_stats <- design %>%}
  summarise(
    across(starts_with("reply_rate"), 
           list(min = min, max = max, mean = mean, range = ~max(.) - min(.)),
           .names = "{.col}_{.fn}")
  )
```
...
```{r}
table(experiment_data$geography, experiment_data$race, experiment_data$gender)
...
print("Regional Discrimination Differences - Scenario 1:")
print(regional1)
print("Regional Discrimination Differences - Scenario 2:")
print(regional2)
print("Regional Discrimination Differences - Scenario 3:")
print(regional3)
```
...
# display results
print("Scenario 1 Discrimination Effects:")
print(disc_effects_s1$race_main)
print(disc_effects_s1$race_x_geography)
```
...
calculate_power <- function(n, scenario_num, n_sims = 100) {
  ...
  cat(sprintf("Scenario %d, n=%d: Power = %.3f\n", scenario_num, n, power))
  return(power)
}
```
...
generate_power_curves <- function() {
  sample_sizes <- c(100, 200, 400, 600, 800, 1000, 1200, 1250, 1400, 1500, 1600, 1750, 1800, 1900, 2000, 5000, 10000)
...
plot_power_curves <- function(power_results) {
  p <- ggplot(power_results, aes(x = sample_size, y = power, 
    ...
    scale_x_continuous(breaks = c(100, 200, 400, 600, 800, 1000, 1200, 1250, 1400, 1500, 1600, 1750, 1800, 1900, 2000, 5000, 10000)) +
    theme_minimal() +
    theme(legend.position = "bottom",
          legend.direction = "vertical",
          plot.title = element_text(size = 14, face = "bold"),
          plot.subtitle = element_text(size = 11, face = "italic"))
  p <- p + annotate("text", x = 1800, y = 0.82, 
                    label = "80% power", 
                    size = 3, 
                    color = "red")
  return(p)
...
generate_summary_report <- function(power_results) {
  ...
  for(s in unique(power_results$scenario_label)) {
    ...
    cat(sprintf("%s:\n", s))
    cat(sprintf("  - Sample size for 80%% power: n = %s\n", power_80_n))
    cat(sprintf("  - Power at n=400: %.1f%%\n", 
                100 * scenario_data$power[scenario_data$sample_size == 400]))
    cat(sprintf("  - Power at n=1000: %.1f%%\n\n", 
                100 * scenario_data$power[scenario_data$sample_size == 1000]))
  }
}
```
...
print_scenario_tables <- function() {
  ...
  all_rates <- data.frame(
    Scenario = rep(c("Service", "Employment", "Political"), each = 8),
